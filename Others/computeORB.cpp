...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
	    int a =0;
	    cv::Point2f zb=kp.pt;
	    if(zb.x<8*image.channels() || zb.x>image.step-8*image.channels()) a=1;
        if(zb.y<8 || zb.y>image.rows-8) a=1;
	    if(a==1){
	        continue;
	    }
	    double m10=0;
	    double m01=0;
	    for(int i=zb.x-8;i!=zb.x+8;i++){
	        for(int j=zb.y-8; j!=zb.y+8;j++){
	            double gray=0.3*image.data[j*image.step+i*image.channels()]+ 0.59*image.data[j*image.step+i*image.channels()+1]+ 0.11*image.data[j*image.step+i*image.channels()+2];
	            m10+=i*gray;
	            m01+=j*gray;
	        }
	    ...
	    double jiao=std::atan(m10/m01);



        kp.angle = pi*jiao; // compute kp.angle
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            int hen=image.step/image.channels();
            int shu=image.cols;
            int flag=0;
            int ux=kp.pt.x;
            int uy=kp.pt.y;

            int up=std::cos(kp.angle*pi)*ORB_pattern[4*i]-std::sin(kp.angle*pi)*ORB_pattern[4*i+1];
            int vp=std::sin(kp.angle*pi)*ORB_pattern[4*i]+std::cos(kp.angle*pi)*ORB_pattern[4*i+1];
            int uq=std::cos(kp.angle*pi)*ORB_pattern[4*i+2]-std::sin(kp.angle*pi)*ORB_pattern[4*i+3];
            int vq=std::sin(kp.angle*pi)*ORB_pattern[4*i+2]+std::cos(kp.angle*pi)*ORB_pattern[4*i+3];

            if(ux+up<0 || ux+up>hen || ux+uq>hen || ux+uq<0){
                flag=1;
            }
            else if(uy+vp<0 || uy+vq>shu || uy+vp>shu || uy+vq<0){
                flag=1;
            }
            if(flag=1) d[i] = 0;

            else{
                double gray1=0.3*image.data[vp*image.step+up*image.channels()]+ 0.59*image.data[vp*image.step+up*image.channels()+1]+ 0.11*image.data[vp*image.step+up*image.channels()+2];
                double gray2=0.3*image.data[vp*image.step+up*image.channels()]+ 0.59*image.data[vp*image.step+up*image.channels()+1]+ 0.11*image.data[vp*image.step+up*image.channels()+2];
                if(gray1<=gray2){
                    d[i]=1;
                }
                else{d[i]=0;}
